...
var mt=document.querySelector('.search__onclick-event');mt.addEventListener('keydown',function(event){if(event.key=='Enter'){if(!mt.checked){mt.checked=!0}else{mt.checked=!1}}else{event.preventDefault()}
if(mt.checked){document.querySelector('.search__entry-data').focus()}else{mt.blur();document.querySelector('.btn_big').focus()}})
document.querySelectorAll('.accordion__tab').forEach(function(et){et.addEventListener('keydown',function(event){if(event.key=='Enter'){var ri=event.target.getAttribute('for');document.querySelectorAll('.accordion__radio').forEach(function(abc){abc.removeAttribute('checked')})
